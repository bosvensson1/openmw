ESMESM#include "npcanimation.hpp"
...
void NpcAnimation::updateParts()
{
    ...
    for(size_t i = 0;i < slotlistsize && mViewMode != VM_HeadOnly;i++)
    {
        ...
        if(store->getType() == typeid(ESM::Clothing::sRecordId)
        {
        ...
        else if(store->getType() == typeid(ESM::Armor::sRecordId)
        {
    ...
    if(mPartPriorities[ESM::PRT_Shield] < 1)
    {
        ...
        if(store != inv.end() && (part=*store).getType() == typeid(ESM::Light::sRecordId)
        {
...
bool NpcAnimation::addOrReplaceIndividualPart(ESM::PartReferenceType type, int group, int priority, const std::string &mesh, bool enchantedGlow, osg::Vec4f* glowColor)
{
    ...
    try
    {
        ...
        if (type == ESM::PRT_Weapon)
        {
            ...
            if(weapon != inv.end() && weapon->getType() == typeid(ESM::Weapon::sRecordId)
            {
...
void NpcAnimation::showWeapons(bool showWeapon)
{
    ...
    if(showWeapon)
    {
        ...
        if(weapon != inv.end())
        {
            ...
            if (weapon->getType() == typeid(ESM::Weapon::sRecordId &&
...
void NpcAnimation::showCarriedLeft(bool show)
{
    ...
    if(show && iter != inv.end())
    {
        ...
        if (iter->getType() == typeid(ESM::Armor::sRecordId)
        {
                                        ...
                                        mesh, !iter->getClass().getEnchantment(*iter).empty(), &glowColor))
        {
            ...
            if (iter->getType() == typeid(ESM::Light::sRecordId && mObjectParts[ESM::PRT_Shield])
